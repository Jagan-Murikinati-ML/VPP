...
-- STEP 4: Add site_owner_authorization to tb_vpp_group_site
ALTER TABLE asset.tb_vpp_group_site 
ADD COLUMN IF NOT EXISTS site_owner_authorization asset.site_authorization_status DEFAULT 'PENDING';

COMMENT ON COLUMN asset.tb_vpp_group_site.site_owner_authorization IS 
'Site owner authorization status for VPP group participation. Values: PENDING, AUTHORIZED, DECLINED. Default: PENDING';
...
WHERE table_schema = 'asset' 
  AND table_name IN ('tb_bas_program_info', 'tb_bas_site', 'tb_vpp_group_site')
